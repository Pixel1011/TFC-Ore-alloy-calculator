...
export function calculateOres(ore: Ores, total?: number, bypass100max = false): OreResults {
    ...
    let max: Ores = {PoorOre: 0, NormalOre: 0, RichOre: 0, sum: 0, Ingots: 0, DoubleIngots: 0, Name: ore.Name};
...
function addOreToString(s: string, ore: Ores) {
...
export function outputCalculated(ore: Ores, total?: number, bypass100max = false) {
    let output = "";
    // i think ingots/double ingots shouldnt be used here, only really for alloys, otherwise your being told to melt ingots for no reason
    ore.DoubleIngots = 0;
    ore.Ingots = 0;
...
export function outputCalculatedMetal(calculated: OreResults) {
    ...
    let leastNormal: Ores | undefined;
    let leastPoor: Ores | undefined;
    let leastIngots: Ores | undefined;
    let leastDoubleIngots: Ores | undefined;

    let maxRich: Ores | undefined;
    let maxNormal: Ores | undefined;
    let maxPoor: Ores | undefined;
    let maxIngots: Ores | undefined;
    let maxDoubleIngots: Ores | undefined;

    if (results.length > 0) {
        calculated.results.forEach(r => {
            if (maxRich == undefined || r.RichOre > maxRich.RichOre && r.RichOre > 0) maxRich = r;
            if (maxNormal == undefined || r.NormalOre > maxNormal.NormalOre && r.NormalOre > 0) maxNormal = r;
            if (maxPoor == undefined || r.PoorOre > maxPoor.PoorOre && r.PoorOre > 0) maxPoor = r;
            if (maxIngots == undefined || r.Ingots > maxIngots.Ingots && r.Ingots > 0) maxIngots = r;
            if (maxDoubleIngots == undefined || r.DoubleIngots > maxDoubleIngots.DoubleIngots && r.DoubleIngots > 0) maxDoubleIngots = r;

            if (leastRich == undefined || r.RichOre < leastRich.RichOre && r.RichOre > 0) leastRich = r;
            if (leastNormal == undefined || r.NormalOre < leastNormal.NormalOre && r.NormalOre > 0) leastNormal = r;
            if (leastPoor == undefined || r.PoorOre < leastPoor.PoorOre && r.PoorOre > 0) leastPoor = r;
            if (leastIngots == undefined || r.Ingots < leastIngots.Ingots && r.Ingots > 0) leastIngots = r;
            if (leastDoubleIngots == undefined || r.DoubleIngots < leastDoubleIngots.DoubleIngots && r.DoubleIngots > 0) leastDoubleIngots = r;
        });
        if (leastRich != undefined) {
            output += "\nLeast rich:\n";
            output = addOreToString(output, leastRich);
...
export function outputCalculatedAlloy(calculated: AlloyOutput) {
    let output = "";
    let ores: Ores[] = [];
    let total = 0;
    calculated.ores.forEach(c => {
        // only 1 result seems to exist
        let res = c.results[0];
    ...
    ores.forEach(t => {
        ...
        if (t != ores[ores.length - 1]) percentages += ", ";
    });
    percentages += ")";
...
export function TwoMetalAlloy(part1:AlloyPart, part2:AlloyPart): AlloyOutput | null {
    let ore1 = part1.ore;
...
export function ThreeMetalAlloy(part1:AlloyPart, part2:AlloyPart, part3:AlloyPart) {
    let ore1 = part1.ore;
    let ore1ratio = part1.ratio;
    let ore2 = part2.ore;
    let ore2ratio = part2.ratio;
        ...
        for (let ore2I = ore2max; ore2I >= 0; ore2I--) {
            for (let ore3I = ore3max; ore3I >= 0; ore3I--) {
                ...
                if (!ore1WithinPercentages) {
                    // this is literally best thing i can think of for speed, decreases accuracy but oh well, im happy (tuned numbers to maybe keep accuracy up, but not sure)
                    ore1I -= 45;
                    continue;
                }
                ...
                if (calculatedOre1.results[0] == undefined || calculatedOre2.results[0] == undefined || calculatedOre3.results[0] == undefined) continue;
